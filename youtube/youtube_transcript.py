...
def find_yt_dlp() -> str:
    """Retourne le chemin de l'exécutable yt-dlp disponible dans le PATH."""
    executable = shutil.which("yt-dlp")

    if executable is None:
        raise TranscriptError(
            "yt-dlp est requis mais aucun exécutable 'yt-dlp' n'a été "
            "trouvé dans le PATH."
        )
...
def download_subtitle_json(
    ...
    """Télécharge et décode la piste JSON3."""
    try:
